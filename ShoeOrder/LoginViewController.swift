...
//                                         "typevat:", customer.typevat ? 1 : 0)
//                            }
                            
                            // ✅ ไม่ต้อง cast เป็น [[String: Any]]
                            ...
                            self.addData(customers: customers)
    ...
    struct Customer: Decodable {
        ...
        enum CodingKeys: String, CodingKey {
            case code, crterm, disc, typevat, curr_ver
            case customer_name = "arname"   //แปลงจาก arname -> customer_name
            case saleman = "id"
            case salename = "name"
        }
    }
        ...
        var db: OpaquePointer?
            ...
            else
            {
                ...
                for c in customers {
                    ...
                    {
                        sqlite3_bind_text(statement, 1, (c.code as NSString).utf8String, -1, SQLITE_TRANSIENT)
                        sqlite3_bind_text(statement, 2, (c.customer_name as NSString).utf8String, -1, SQLITE_TRANSIENT)
                        sqlite3_bind_int(statement, 3, Int32(exactly: c.crterm)!)
                        sqlite3_bind_double(statement, 4, c.disc)
                        sqlite3_bind_text(statement, 5, c.saleman, -1, SQLITE_TRANSIENT)
                        sqlite3_bind_text(statement, 6, c.salename, -1, SQLITE_TRANSIENT)
                        sqlite3_bind_int(statement, 7, Int32(exactly: c.typevat ? 1 : 0)!)
                    
                        CustomerViewController.GlobalValiable.saleid = c.saleman   //เก็บ รหัส sale
